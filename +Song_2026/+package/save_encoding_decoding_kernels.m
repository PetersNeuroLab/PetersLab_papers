...
for curr_animal_group = 1:length(animals)
    for curr_animal_idx = 1:length(animals{curr_animal_group})
        ...
        for curr_recording = rec_days_unimodal_idx'
            ...
            wf_V_measured = wf_V(1:n_components,skip_frames:end-skip_frames);
            ...
            % Get explained variance from encoding regression
            % expl_var = (1-var(y-y_pred)/var(y), with y_pred NaNs as component mean
            explained_var_stim = 1 - var(wf_V_measured- ...
                fillmissing(predicted_signals_stim','constant',nanmean(wf_V_measured,2))',[],'all')./ ...
                var(wf_V_measured,[],'all');

            explained_var_move = 1 - var(wf_V_measured- ...
                fillmissing(predicted_signals_move','constant',nanmean(wf_V_measured,2))',[],'all')./ ...
                var(wf_V_measured,[],'all');

            % Decoding: regress to stim and move (on full and residuals)
            kernels_stim_decode_full = ...
                ap.regresskernel(wf_V_measured, ...
            ...
            encoding_decoding_kernels(curr_animal_group).explained_var_stim{curr_animal_idx}(curr_recording) = explained_var_stim;
            encoding_decoding_kernels(curr_animal_group).explained_var_move{curr_animal_idx}(curr_recording) = explained_var_move;
