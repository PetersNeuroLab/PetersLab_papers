...
plot_grp_order = [0,0;0,1;1,0;1,1];

grp_idx = [modality_idx_cat(use_kernels),learned_idx_cat(use_kernels)];
[~,grp_idx_num] = ismember(grp_idx,plot_grp_order,'rows');

figure;
boxchart(repmat(grp_idx_num,2,1),[full_moveres_corr;full_stimres_corr], ...
    'GroupByColor',categorical(vertcat(repelem("Move-residual",length(full_moveres_corr),1), ...
    repelem("Stim-residual",length(full_moveres_corr),1))));

ylabel('Spatial correlation');
set(gca,'XTick',1:size(plot_grp_order,1),'XTickLabel',arrayfun(@(x) ...
    sprintf("mod. %d, learn. %d", ...
    plot_grp_order(x,:)),1:size(plot_grp_order,1)), ...
