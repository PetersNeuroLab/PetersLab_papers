...
temp_num=cellfun(@(s) feval(@(a)  cat(2,a{:}) ,cellfun(@(x) ...
    nanmean(x,2), s.kernels_data.trial_number,'UniformOutput',false)),tempdata,'uni',false     );
...
temp_num_mean=feval(@(a) cat(2,a{:}),  cellfun(@(x) nanmean(x,'all')   ,temp_num,'UniformOutput',false));
temp_num_error=feval(@(a) cat(2,a{:}), cellfun(@(x) nanstd(nanmean(x,2),0,1)./sqrt(size(x,1))   ,temp_num,'UniformOutput',false));
...
%     'Lab\Papers\Song_2026\submission_3_NatureCommunications\revisions\revision_figures\eps\Fig_EDF_F1.eps'), ...
%     'ContentType','vector');
% %
