...
%%  figure 6c probes_corornal slices
main_preload_vars=who;
bregma=[520,44,570];
groups={'VA','AV'}
...
%% figure 6d,e,f,g  psth of all cells in passive
main_preload_vars = who;
...
%% figure 6h-j group average in passive by depth
main_preload_vars = who;
...
%% figure 6k scatter of visual vs auditory 
main_preload_vars = who;
...
for curr_group=1:2
    ...
    response_both_shuff_sep= cellfun(@(x)  arrayfun(@(shuff) ap.shake(x,1),1:n_shuff,...
        'UniformOutput',false),response_sort_both,'UniformOutput',false);

    response_both_fration_shuff_sep=cellfun(@(a)  nanmean( cellfun(@(x)    nanmean( sum(x,2)==2),...
        a,'UniformOutput',true)),response_both_shuff_sep,'UniformOutput',true);

   % p_vals{curr_group}=ds.shuffle_test(response_both_fration_shuff_sep,response_both_fration_both{curr_group},0,1);
   
   response_multimodal_avg_shuff = ...
       mean(cell2mat(cellfun(@(animal) cellfun(@(day) ...
       mean(sum(day,2)==2),animal),response_both_shuff_sep,'uni',false)),1);
   line_mean_both{curr_group}= (prctile(response_multimodal_avg_shuff,95)+ prctile(response_multimodal_avg_shuff,5))/2;
   line_error_both{curr_group}=(prctile(response_multimodal_avg_shuff,95)- prctile(response_multimodal_avg_shuff,5))/2;

   % response_multimodal_avg = mean(response_both_fration_both{1},1);
   % 
   % response_both_shuff_avg = mean(cell2mat(response_both_shuff_sep),1);
...
for curr_stage=1:2
    ...
    ds.make_bar_plot(bar_temp,'ColorCell' ,colors','BarAlpha'  ,0.2,'DotSize' ,maker_size);
    ylim([0 0.8])
    yticks([0 0.8])
    set(gca,'color','none')
    xticks([])
