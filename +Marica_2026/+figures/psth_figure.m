...
for curr_workflow = plot_workflows
    ...
    for curr_raster = 1:length(raster)
        ...
        % Sub-sample to only plot max of n trials        
        raster_trials_n = 50;
        raster_trials_plot = unique(round(linspace(1,size(raster{curr_raster},1),raster_trials_n)));

        [raster_y,raster_x] = find(raster{curr_raster}(trial_sort{curr_raster}(raster_trials_plot),:));
        plot(raster_t(raster_x),raster_y,'.','color',curr_colors{curr_raster});
        xlim(raster_window)
        axis off
        set(gca,'YDir','reverse');
        line(repelem(0,2),[0,length(raster_trials_plot)],'color','r','linewidth',1);
