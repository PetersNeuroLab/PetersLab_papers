...
for animal_idx=1:length(animals)
    ...
    for use_rec=1:length(recordings_task)
        ...
        [unit_stim_psths,~] = ap.psth(spike_times_timelite,stimOn_times(trial_outcome==1 & trial_opacity==1),spike_templates);
        [unit_move_psths,~] = ap.psth(spike_times_timelite,stim_move_time(trial_outcome==1 & trial_opacity==1),spike_templates);
        [unit_outcome_psths,~] = ap.psth(spike_times_timelite,stimOff_times(trial_outcome==1 & trial_opacity==1),spike_templates);

        [unit_stim_nostim_psths,~] = ap.psth(spike_times_timelite,stimOn_times(trial_outcome==1 & trial_opacity==0),spike_templates);
        [unit_move_nostim_psths,~] = ap.psth(spike_times_timelite,stim_move_time(trial_outcome==1 & trial_opacity==0),spike_templates);
        [unit_outcome_nostim_psths,~] = ap.psth(spike_times_timelite,stimOff_times(trial_outcome==1 & trial_opacity==0),spike_templates);

        unit_event_psths = cat(3,unit_stim_psths,unit_move_psths,unit_outcome_psths, ...
            unit_stim_nostim_psths,unit_move_nostim_psths,unit_outcome_nostim_psths);
