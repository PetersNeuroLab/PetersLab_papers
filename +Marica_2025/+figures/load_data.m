...
% Before loading, clear workspace except for fig settings (unless flag)
if ~exist('load_dataset_retain','var') || ~load_dataset_retain
...
if ~strcmp(load_dataset,'noact')
    ...
    % % mua_baseline = cellfun(@(mua,domain_idx) ...
    % %     mean(ap.groupfun(@sum,mua,[],[],domain_idx),[1,2]), ...
    % %     cellfun(@(x) x(:,baseline_t,:,1),ephys_task.ephys.binned_msn_spikes_event_align, ...
    % %     'uni',false,'ErrorHandler',@(varargin) []), ...
    % %     domain_idx_rec,'uni',false);
    % % clear ephys_task % clear - only used for baseline
    % % % (option 2: recording baseline)
    % % mua_baseline = cellfun(@(mua) ...
    % %     mean(mua(:,baseline_t,:,1),[1,2]), ...
    % %     striatum_mua_sum,'uni',false,'ErrorHandler',@(varargin) NaN);
    % % % (option 3: trial baseline)
    % % mua_baseline = cellfun(@(mua) ...
    % %     mean(mua(:,baseline_t,:,1),[2]), ...
    % %     striatum_mua_sum,'uni',false,'ErrorHandler',@(varargin) NaN);
    % % % (option 4: moving median trial baseline)
    % % mua_baseline = cellfun(@(mua) ...
    % %     movmedian(mean(mua(:,baseline_t,:,1),[2]),20,1), ...
    % %     striatum_mua_sum,'uni',false,'ErrorHandler',@(varargin) NaN);
    % 
    % % Define baselines
    % % (subtract pre-stim average for each trial)
    % mua_sub_baseline = cellfun(@(mua) ...
    %     mean(mua(:,baseline_t,:,1),[2]), ...
    %     striatum_mua_sum,'uni',false,'ErrorHandler',@(varargin) NaN);
    % 
    % % (divide by mean pre-stim time during task)
    % ephys_task = load(fullfile(data_path,'ephys_task'));
    % mua_div_baseline = cellfun(@(mua,domain_idx) ...
    %     mean(ap.groupfun(@sum,mua,[],[],domain_idx),[1,2]), ...
    %     cellfun(@(x) x(:,baseline_t,:,1),ephys_task.ephys.binned_msn_spikes_event_align, ...
    %     'uni',false,'ErrorHandler',@(varargin) []), ...
    %     domain_idx_rec,'uni',false);
    ...
    % 
    % mua_norm_smooth_reshape_fcn = @(spikes,sub_baseline,div_baseline) ...
    %     reshape(permute(smoothdata((spikes-sub_baseline)./(div_baseline+softnorm),2, ...
...
if ~strcmp(load_dataset,'noact')
    ...
    sua_norm_smooth_reshape_fcn = @(spikes,baseline) ...
    ...
    striatum_sua = cell2mat(cellfun(@(data,baseline,striatum_units) ...
        sua_norm_smooth_reshape_fcn(data(striatum_units,:,:),baseline(striatum_units)), ...
...
% %%%% TESTING: manual ROIs
% warning('MANUAL ROIS');
% load("C:\Users\petersa\Desktop\striatum_roi_manual.mat");
% striatum_wf_roi = striatum_roi_manual;
% %%%
...
if ~strcmp(load_dataset,'noact')
    ...
    % (baseline-subtract)
    baseline_t = wf_t < 0;
    wf_striatum_roi = wf_striatum_roi - nanmean(wf_striatum_roi(:,baseline_t,:,1),2);
